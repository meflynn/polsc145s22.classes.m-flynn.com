...
dfr <- data.frame(date=seq(as.Date('2022-01-17'), as.Date('2022-05-31'), by = 1)) %>% 
  mutate(day = factor(strftime(date, format="%a"), levels=c("Sun","Mon","Tue","Wed","Thu","Fri","Sat")),
         ...
         comment = case_when(
           date == as.Date("2022-02-10") ~ "Exam 1",
           date == as.Date("2022-03-28") ~ "Exam 2",
         ...
         week - as.numeric(week) - as.numeric(week) - 1)
...
## `r advdate(mon, 7)`: Military Intervention and Peacekeeping
...
## `r advdate(mon, 8)`: Civil War and non-state actors
...
## `r advdate(mon, 11)`: International Trade
...
## `r advdate(mon, 12)`: International Financial Relations
...
## `r advdate(mon, 13)`: International Monetary Relations
...
## `r advdate(mon, 14)`: International Development
...
## `r advdate(mon, 16)`: Global Environmental Politics
